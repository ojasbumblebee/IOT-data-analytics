...
os.chdir("/home/ojas/Documents/independent_study_harry_perros/data/daily")
time_series_data = load_csv_data(filename="tmp112_temperature_chicago-complete.daily.2018-12-03.csv")

time_series_data = time_series_data.loc[time_series_data["node_id"] == time_series_data.node_id[0]]

df = time_series_data[["value_hrf"]]
print(df.keys())
df = df.rename(columns={'value_hrf': 'time_series'})
#df = pd.DataFrame(np.array(time_series_data), columns = ["value_hrf"])
train_data, test_data = train_test_split(df, test_size=0.25, shuffle=False)
#print(df)
os.chdir("/home/ojas/Documents/independent_study_harry_perros/time_series_forecasting/graphs")
X = df.time_series
result = adfuller(X)
